...
existing row; only what's left unpaired is deleted (which cascades
...
every query over that table had to exclude them.
...
not assumed: the failure people hit is`Sign in to confirm you're not
